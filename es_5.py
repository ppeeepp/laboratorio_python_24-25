...
def is_valid(pos):
    ...
    :param pos: lista di interi -> l'indice è la riga, il valore la colonna
...
def rotate_90(solution):
    ...
    for r, c in enumerate(solution):
        # la regina in (r, c) va in (c, n-1-r) dopo rotazione di 90°
        rotated[c] = n - 1 - r
    return rotated
    ...
    try:                                                            # controllo dei parametri di ingresso
        if n > 0 and n_solutions > 0:
            ...
            while len(unique_solutions) < n_solutions:
                ...
                if is_valid(list_of_solutions):
                    rotated_solutions = get_all_rotations(list_of_solutions)        # ottengo tutte le rotazioni delle soluzioni
                    tuples_of_solutions = [tuple(r) for r in rotated_solutions]     # converto le liste in tuple per poterla aggiungere al set
                    already_seen = False                                            # controllo che una soluzione non venga ripetuta
